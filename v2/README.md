...
The V2 now references the original root assets through relative paths. This avoids duplication in `/v2/` while restoring the SATONA product photography, logos, PDF and original plate SVG. The historic SVG keeps the `w{line}-b{bit}` identifiers consumed by `plate-preview.js`.

The V2 download controls point to the existing root PDF through relative paths; the PDF is not copied into `/v2/`.
...
- No binary visual asset is stored in `v2/assets/images/`; pages reference the existing root `assets/images/` directory using relative URLs.
